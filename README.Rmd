...
https://github.com/mtennekes/tmap/blob/master/pkg/R/bb.R
plus
https://github.com/mtennekes/tmap/blob/master/pkg/R/end_of_the_world.R
plus `maybe_longlat` from 
https://github.com/mtennekes/tmap/blob/master/pkg/R/is_projected.R
plus raster::extent, rgeos::gIntersection
...
summary (issue_query (q3))
```

```{r junk2}
require (sp)
sldf <- new ("SpatialLinesDataFrame")
sldf
junk <- list (lines1=sldf, lines2=sldf)
junk
summary (junk)
```
...
```{r only_nodes, cache=TRUE, eval=TRUE}
# just nodes
only_nodes <- '[out:xml];
node
  ["highway"="bus_stop"]
...
```{r nodes_and_ways, cache=TRUE, eval=TRUE}
# ways & nodes
...
```{r actual_ways, cache=TRUE, eval=TRUE}
