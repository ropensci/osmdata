...
[![R CMD check](https://github.com/ropensci/osmdata/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/ropensci/osmdata/actions/workflows/R-CMD-check.yaml)
...
These reflect the four locations at which the source code of this package is
currently hosted:
...
The first of these, [github.com/ropensci/osmdata](https://github.com/ropensci/osmdata), is currently the primary location for the source code, and any questions or suggestions should preferably be directed there, although we will also respond to comments or issues on any of the other platforms.
...
built from a base query constructed with `opq` followed by `filter_osm_tags`. The
...
The next step is to define features of interest using the [`filter_osm_tags()`
function](https://docs.ropensci.org/osmdata/reference/filter_osm_tags.html).
This function accepts `key` and `value` parameters specifying the tags of the
desired features in the
[OSM key-vale schema](https://wiki.openstreetmap.org/wiki/Map_Features).
Multiple `filter_osm_tags()` calls may be combined as illustrated below, with
