...
download and process OSM data. Raw data as returned directly from the
[overpass API](https://overpass-api.de).  can be downloaded with `osmdatar` with
this code:
```{r, eval=FALSE}
...
processes OSM data. To time just the processing stage, the relevant code has to
be re-created here (hard-coded for `key=highway`):
...
To enable different elements to be extracted from a single download, `osmdatar`
allows raw data to first be downloaded prior to re-submission to the same
function for subsequent parsing. The `osmdatar` speed test thus simply requires:
```{r, eval=FALSE}
