...
```{r}
doc <- xml2::read_xml ("export.osm", encoding="UTF-8")
res <- process_doc (as (doc, "character"))
summary (res$osm_polygons)
df <- slot (res$osm_polygons [[1]], "data")
indx <- as.numeric (which (apply (df, 2, function (i) 
                                  length (table (i, useNA="always")) > 1)))
names (df) [indx]
names (df) [which (!seq (ncol (df)) %in% indx)]
indx2 <- which (!seq (ncol (df)) %in% indx)
slot (res$osm_polygons [[1]], "data") <- df [,indx]
```
