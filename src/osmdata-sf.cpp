        ...
        const Rcpp::NumericVector &bbox, const Rcpp::List &crs)
{
    ...
    Rcpp::CharacterMatrix meta_mp (Rcpp::Dimension (nmp, 5L)),
        meta_ls (Rcpp::Dimension (nls, 5L));
    std::fill (meta_mp.begin (), meta_mp.end (), NA_STRING);
    std::fill (meta_ls.begin (), meta_ls.end (), NA_STRING);
    ...
    for (auto itr = rels.begin (); itr != rels.end (); ++itr)
    {
        ...
        if (itr->ispoly) // itr->second can only be "outer" or "inner"
        {
            ...
            meta_mp (count_mp, 0L) = itr->_version;
            meta_mp (count_mp, 1L) = itr->_timestamp;
            meta_mp (count_mp, 2L) = itr->_changeset;
            meta_mp (count_mp, 3L) = itr->_uid;
            meta_mp (count_mp, 4L) = itr->_user;
        ...
        } else // store as multilinestring
        {
            ...
            for (std::string role: roles)
            {
                ...
                meta_ls (count_ls, 0L) = itr->_version;
                meta_ls (count_ls, 1L) = itr->_timestamp;
                meta_ls (count_ls, 2L) = itr->_changeset;
                meta_ls (count_ls, 3L) = itr->_uid;
                meta_ls (count_ls, 4L) = itr->_user;
    ...
    for (std::string i: not_okay_id)
    {
        ...
        for (int k = 0; k < kv_mat_mp.nrow (); k++)
        {
            if (k < j)
            {
                ...
                meta_mp2 (k, Rcpp::_) = meta_mp (k, Rcpp::_);
            } else if (k > j)
            {
                kv_mat_mp2 (k - 1, Rcpp::_) = kv_mat_mp (k, Rcpp::_);
                meta_mp2 (k - 1, Rcpp::_) = meta_mp (k, Rcpp::_);
            }
        }
    ...
    if (rel_id_ls.size () > 0) // only if there are linestrings
    {
        ...
        meta_ls.attr ("dimnames") = Rcpp::List::create (rel_id_ls, metanames);
    } else
    {
        kv_df_ls = R_NilValue;
        meta_ls = R_NilValue;
    ...
    if (rel_id_mp.size () > 0)
    {
        ...
        meta_mp.attr ("dimnames") = Rcpp::List::create (rel_id_mp, metanames);
    } else
    {
        kv_df_mp = R_NilValue;
        meta_mp = R_NilValue;
