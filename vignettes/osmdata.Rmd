...
`osmdata` aims to facilitate access and use of OSM with R.
...
'features', which are specified in terms of `key-value` pairs (see 
...
Distinct features must be requested with different queries, so it makes sense to
give each query a separate name, e.g.:
...
The two functions `osmdata_sf` and `osmdata_sp` both return objects of class
`osmdata`, as described in detail below.
...
```{r}
cpaths_sev = rbind(
  cway_sev_bicycle$osm_lines[1:3],
  cway_sev_bridge$osm_lines[1:3]
  )
...
The OSM data can then be saved as an XML document with the following command:
...
and processing spatial data: `Spatial` from the `sp` package and `Simple Features` from the `sf` package.
