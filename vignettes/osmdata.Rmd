...
`opq_to_string()`.  Applied to the preceding query, this function gives:
...
(The output of `opq_to_string` has been somewhat reformatted here to reflect
...
such as those produced by the `osmdata` function `opq_to_string()`, or copied
...
Nevertheless, the results of the two calls `osmdata_sf (opq_to_string (q))` and
