...
`osmdata` is an R package for downloading and using data from OpenStreetMap ([OSM](www.openstreetmap.org/)).
OSM is a global open access mapping project, which is free and open under the [ODbL licence](http://www.openstreetmap.org/copyright) [@OpenStreetMap].
This has many benefits, ensuring transparent data provenance and ownership, enabling real-time evolution of the database and, by allowing anyone to contribute, encouraging democratic decision making and citizen science [@johnson_models_2017].
See the [OSM wiki](http://wiki.openstreetmap.org/wiki/Contribute_map_data) to find out how to contribute to the world's open geographical data commons. 
...
(typically of class `sf`) or
Spatial objects (e.g. `SpatialPointsDataFrame`), as defined
by the packages [`sf`](https://cran.r-project.org/package=sf) and [`sp`](https://cran.r-project.org/package=sp) packages respectively.

`overpass` is a C++ library that serves OSM
data over the web.  All `overpass` queries begin with a bounding box, defined in
`osmdata` with the function `opq()`:
...
`SpatialLinesDataFrame` objects (with this particular query returning no
`polygons` or `multi` objects).
...
`qry_to_string()`.  Applied to the preceding query, this function gives:
...
(The output of `qry_to_string` has been somewhat reformatted here to reflect
...
such as those produced by the `osmdata` function `qry_to_string()`, or copied
directly from the [`overpass-turbo` query builder](overpass-turbo.eu).

```{r, eval=FALSE}
osmdata_sf(qry_to_string(q))
```
...
Nevertheless, the results of the two calls `osmdata_sf(qry_to_string(q))` and
...
Queries are, however, usually more useful when refined through using `add_feature()`, which
minimally requires a single `key` and returns all objects specifying any value
for that `key`: 
...
`AND`, showing the utility of combining `osmdata` objects with the generic function `c()`. 

## 3. The `osmdata` object

The `osmdata` extraction functions (`osmdata_sf()` and `osmdata_sp()`), both
return objects of class `osmdata`.
The structure of `osmdata` objects are clear from their default print method, illustrated using the `bridge` example from the previous section:
...
These some or all of these can be empty: only points and lines are illustrated in the example printed above. The more complex features of `osm_multilines` and `osm_multipolygons` refer to OSM relations than contain multiple lines and polygons.

  The actual spatial data contained
in an `osmdata` object are of either `sp` format when extracted with
`osmdata_sp()` or `sf` format when extracted with `osmdata_sf()`. 
...
"  <meta osm_base=\"2017-03-07T09:28:03Z\"/>",
"  <node id=\"21593231\" lat=\"51.5149566\" lon=\"-0.1134203\"/>",
"  <node id=\"25378129\" lat=\"51.5135870\" lon=\"-0.1115193\"/>",
"  <node id=\"25496793\" lat=\"51.5176768\" lon=\"-0.1111129\"/>",
"  <node id=\"25496827\" lat=\"51.5164528\" lon=\"-0.1093186\"/>")
```
...
```{r, echo=FALSE}
msg <- c("Reading layer `points' from data source `trentham.osm' using driver `OSM'\n",
...
```{r, eval=FALSE}
names(sf::st_read('trentham.osm', layer='points', quiet=TRUE)) # these are the keys
```
...
than equivalent `sf/GDAL` routines (as detailed [here](git-wiki)).
...
```{r, echo=FALSE}
message('[1] "SpatialPolygonsDataFrame"\nattr(,"package")\n[1] "sp"')
```
...
```{r, echo=FALSE}
msg <- c("Simple feature collection with 1 feature and 3 fields\n",
...
```{r, echo=FALSE}
msg <- c("Simple feature collection with 1 feature and 4 fields\n",
"geometry type:  POINT\n",
"dimension:      XY\n",
