...
Unlike the [`OpenStreetMap`](https://cran.r-project.org/web/packages/OpenStreetMap/index.html) package,
which facilitates the download of raster tiles, `osmdata` provides access to the vector data underlying OSM.
...
Bounding boxes may also be defined by simply passing the name of a desired area - if `bbox` is a text string `opq()` uses the helper function `getbb()` to convert it into a bounding box:
...
Note that the text string is not case sensitive, and that you can avoid the intermediate step of using `getbb()` by placing a text string directly into the `bbox` argument of `opq()`:
...
By default, `osmdata` sends queries to `http://overpass-api.de/api/interpreter` but other servers can be used, thanks to functions that *get* and *set* the base url:
