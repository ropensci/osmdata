...
'tags', which are specified in terms of `key-value` pairs.  For example, all
...
q <- opq (bbox = "greater london uk") |>
    filter_osm_tags (key = "highway", value = "motorway")
```
```{r, echo = FALSE}
...
with the function `opq()`, followed by specifying desired OSM tags with
`filter_osm_tags()`.
...
q <- opq (bbox = "Kunming, China") |>
    filter_osm_tags (key = "natural", value = "water") |>
    filter_osm_tags (key = "name:en", value = "Dian", value_exact = FALSE)
```
```{r, echo = FALSE}
q <- opq (bbox = c (102.5417638, 24.8915153, 102.8617638, 25.2115153)) |>
    ...
    filter_osm_tags (key = "name:en", value = "Dian", value_exact = FALSE)
```
Each successive call to `filter_osm_tags()` **adds** tags to a query. This query
...
Single queries are always constructed through **adding** tags, and therefore
...
```{r add_osm_tags-fakey, eval = FALSE}
q <- opq (bbox = "Kunming, China") |>
...
Note that the `"="` symbols here requests tags whose values exactly match
...
[the `add_osm_tags()`
function](https://docs.ropensci.org/osmdata/reference/add_osm_tags.html),
the single-tag version of 
[`filter_osm_tags()`
function](https://docs.ropensci.org/osmdata/reference/filter_osm_tags.html) has
...
with `opq()`, and then specifying OSM tags in terms of `key-value` pairs
with `filter_osm_tags()`, along with judicious usage of the `key_exact`,
...
`filter_osm_tags()`, which minimally requires a single `key` and returns all objects
...
q1 <- opq ("Sevilla") |>
    filter_osm_tags (key = "highway", value = "cycleway")
...
q2 <- filter_osm_tags (q1, key = "bridge", value = "yes")
...
q2 <- opq ("Sevilla") |>
    filter_osm_tags (key = "bridge", value = "yes")
...
`filter_osm_tags()` calls and an extraction function (`osmdata_sf/sp/sc/data_frame/xml`)
