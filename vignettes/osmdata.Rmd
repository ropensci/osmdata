...
```{r}
doc <- overpass_query(qry_to_string(q), quiet = F)
```

This downloads the raw data underlying OSM, in xml format by default.
Let's glimpse the first 100 characters of this dataset:

```{r}
length(doc) # one line of raw text
nchar(doc) # containing almost 1 million chars!
substr(doc, 1, 500) # first 500 chars
```

If you want to use this dataset in another program, like the excellent open source application [QGIS](http://qgis.org/en/site/), you can do so by saving this dataset as a .xml file, e.g.:

```{r}
writeLines(doc, file.path(tempdir(), "doc.osm"))
```

In QGIS this can then be loaded via a two-step process: `Vector > OpenStreetMap > Import topology from XML...` followed by `Vector > OpenStreetMap > Export topology to SpatialLite...`, as illustrated in the figure below:

![](https://cloud.githubusercontent.com/assets/1825120/23326333/7189dd04-faf4-11e6-91f1-bd1b0c366cad.png)

Raw OSM data can also be loaded and edited with the excellent [JOSM](http://wiki.openstreetmap.org/wiki/JOSM) application.
...
The osm doc can extracted as a `Spatial` object defined by the `sp` package, with:

```{r osmdata_sp}
# osmdata_sp (doc = doc) # not working - any ideas?
```

Note that you can also pass queries directly to the `osmdata_sp` function:

```{r}
osmdata_sp(q)
```

or an `R Simple Features` objects with
```{r osmdata_sf}
osmdata_sf (q)
```
