...
OSM is a global open access mapping project, which anyone can contribute to or
access data from [@OpenStreetMap]--see the main website at 
...
The syntax of `overpass` queries, which is powerful yet not easy to learn.
Although it is not necessary to understand this syntax to use `osmdata`, 
reading this section is likely to help construct more efficient and powerful
queries.  Those wanting to skip straight onto query construction in `osmdata`
may safely jump ahead to the [query example below](#query-example).
...
`AND`. (Note that the `c` operator in the above code currently generates a
warning which arises in `sf`, not `osmdata`.)
...
editor [JOSM](http://wiki.openstreetmap.org/wiki/JOSM). 
...
`osm_point` objects describes this waterfall.  The information necessary to
locate this waterfall is removed in the `GDAL/sf` representation, but can be
extracted with `osmdata` with the following lines, noting that the 
OSM ID of the line `li` is given by `rownames (li)`.
```{r, eval=FALSE}
pts <- osm_points (tr, rownames (li))
