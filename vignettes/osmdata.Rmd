...
The `osmdata` R package can be installed from CRAN with
```{r, eval = FALSE}
...
This query is perhaps easier to understand through reformatting to reflect
...
### 2.1 Different `overpass` API servers

By default, `osmdata` uses the main `overpass` API server at
`http://overpass-api.de/api/interpreter`. This default is set when the
`osmdata` package is loaded into the current `R` workspace.  A list of available
servers is maintained at
[http://wiki.openstreetmap.org/wiki/Overpass_API](http://wiki.openstreetmap.org/wiki/Overpass_API).
Alternative servers may be set using `set_overpass_url()`, for example,
```{r, eval = FALSE}
set_overpass_url ('http://overpass.osm.rambler.ru/cgi/interpreter')
```
All subsequent usage of `osmdata` within the same `R` session will then utilise
the nominated overpass API server. A list of alternative servers is also given
in the documentation for `set_overpass_url()` - see `?set_overpass_url`.
...
q <- opq(bbox = c(102.5417638, 24.8915153, 102.8617638, 25.2115153)) %>%
    ...
    add_feature(key = 'name:en', value = 'Dian', value_exact = FALSE)
```
Each successive call to `add_feature()` **adds** features to a query. This query
...
q <- opq(bbox = 'Kunming, China') %>%
    ...
    add_feature(key = 'name:en', value = 'Dian', value_exact = FALSE)
```
The parameter `value_exact = FALSE` controls the two ways of matching the
specified `value` in `osmdata`: Either exactly and entirely, or through a
partial string match.  With the default `value_exact=TRUE`, this query would
only return objects named 'Dian', and not, for example, any named 'Lake Dian'.
Using `value_exact = FALSE` will return 'Lake Dian' through just specifying
`value='Dian'`.

In addtion to `value_exact`, `add_feature` also offers the additional two
...
### 3.2 Extracting `OSM` data from a query
...
with `add_feature()`, along with judicious usage of the `key_exact`,
`value_exact`, and `match_case` parameters.

It is of course possible to simply request all data within a given
...
cycleways or those which are bridges. As described above, combining these into
one query will return only those that are designated cycleways **AND** that are
bridges:
...
And demonstrated above, the `OR` operation produces more data than the equivalent
`AND` operation.
...
with a new query for a small portion of central London, U.K.
...
The following example is based on this query, which extracts all named objects
and saves them in the specified file.
