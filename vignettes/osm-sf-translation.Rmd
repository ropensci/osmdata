...
[OpenStreetMap (OSM)](https://openstreetmap.org) data has a unique structure that
is not directly reconcilable with other modes of representing spatial data,
notably including the widely-adopted 
[Simple Features (SF)](https://www.opengeospatial.org/standards/sfa) 
scheme of the 
...
[described at length by the OGC](https://www.opengeospatial.org/standards/sfo),
...
data](https://www.gdal.org/drv_osm.html), and thus `sf` can also be used to read
`OSM` data in `R`, 
