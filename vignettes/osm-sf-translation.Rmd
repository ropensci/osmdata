---
title: "2. Translation of OSM to Simple Features"
author: "Mark Padgham"
date: "`r Sys.Date()`"
output:
      rmarkdown::html_vignette:
        ...
        toc: true
vignette: >
  %\VignetteIndexEntry{2. OSM to Simple Features}
...
notably including the widely-adopted Simple Features scheme of the Open
...
OSM data are only ever `XY` coordinates represented by a string of `X`
coordinates contained within a bracket, followed by a string of `Y` coordinates.
In addition to coordinate data and associated coordinate reference systems, an
SF may include any number of additional data which quantify or qualify the
feature of interest.  In the 
[`sf` extension to `R`](https://cran.r-project.org/package=sf), for example, a
single SF is represnted by one row of a `data.frame`, which the geometry stored
...
[Geospatial Data Abstraction Library](http:www.gdal.org) (`GDAL`; described
...
properties of the contituent ways of a given OSM relation are stored with those
...
`OSM` data in `R`, 
[as detailed in the main `osmdata` vignette](https://github.com/osmdatar/osmdata/blob/master/vignettes/osmdata.Rmd).
...
Apart from the numerical difference arising through `osmdata` returning an `sf::POINTS` structure containing **all** nodes within a
given set of OSM data, while `sf::st_read (file, layer='points')` returns only
those points not represeted in other structure, the representation of points
remains otherwise broading similar. The only other major difference is that
`osmdata` retains all `key-value` pairs present in a given set of OSM data,
whereas `GDAL/sf` only retains a select few of these. Moreover, the `keys`
returned by `GDAL/sf` are pre-defined and invariant, meaning that data returned
from `sf::st_read (...)` may often contain `key` columns in the resultant
`data.frame` which contain no (non-`NA`) data. This difference is illustrated in
an example repeated here from the  
[main `osmdata` vignette](https://github.com/osmdatar/osmdata/blob/master/vignettes/osmdata.Rmd),
with the same principles applying to all of the following classes of OSM data.

The following three lines define a query and download the resulant data to an
`XML` file.
...
The `osmdata` translation thus readily enables a singlularly complete network to
